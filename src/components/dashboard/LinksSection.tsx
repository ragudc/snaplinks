...
// --- Skeleton for a single link card --------------------------------
...
export function LinksSection({ initialLinks, initialCount }: LinksSectionProps) {
  const { t }               = useTranslation()
  const [search, setSearch] = useState("")
  const debouncedSearch     = useDebounce(search, 300)

  const { links, totalCount, isLoading, error, updateLink, deleteLink, toggleActive } =
    useLinks({ search: debouncedSearch, initialLinks, initialCount })
